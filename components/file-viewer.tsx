...
function Tree({
  // elements is part of public API but not used internally
...
export default function ComponentFileViewer({
  ...
  const tree = useMemo(() => {
    ...
    const root: Record<string, InternalTreeNode> = {};
    ...
    for (const file of files) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        const node = current[part] as InternalTreeNode;
        current = node.children ?? {} as Record<string, InternalTreeNode>;
      }
    }
    const toArray = (obj: Record<string, InternalTreeNode>): TreeViewElement[] =>
      Object.values(obj).map((item: InternalTreeNode): TreeViewElement => ({
        id: item.id,
        name: item.name,
