...
import { getMemoryIndex, isPineconeAvailable, tripPineconeCircuit } from '../database/pinecone';
...
export async function storeDebugMemory(input: StoreDebugMemoryInput): Promise<string> {
  // If DB is not configured, return a fake session ID (memory-only mode)
  if (!isDatabaseConfigured()) {
    console.log('[Storage] Database not configured — skipping persistent storage');
  ...
  await prisma.user.upsert({
    ...
    console.error('[Storage] DB user upsert failed:', dbError);
    return `local-${Date.now()}`;
  }
  ...
  try {
    session = await prisma.debugSession.create({
      data: {
        userId: input.userId,
    ...
    console.error('[Storage] DB session create failed:', dbError);
    return `local-${Date.now()}`;
  }

  // 4. Store vector in Pinecone (skip if not available)
  if (!isPineconeAvailable()) {
    console.log('[Storage] Pinecone not available — skipping vector storage');
  } else {
  ...
  try {
    ...
    console.error('[Storage] Pinecone upsert failed:', (error as Error).message);
    tripPineconeCircuit();
  }
  } // end isPineconeAvailable block
