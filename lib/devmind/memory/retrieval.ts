...
import { isDatabaseConfigured } from '../database/postgres';
...
export async function retrieveMemoryContext(
  ...
  // If neither DB nor Pinecone is configured, return empty context
  if (!isDatabaseConfigured() && !isPineconeAvailable()) {
    console.log('[Retrieval] No external services configured — returning empty context');
    return { similarPastErrors: [], recurringMistakes: [], knownWeakConcepts: [], totalSessionCount: 0, recentErrorTypes: [] };
  }
...
async function findSimilarErrors(
  ...
  if (!isPineconeAvailable()) {
    console.log('[Retrieval] Pinecone not available — skipping vector search');
    return [];
  }
  try {
    ...
    console.error('[Retrieval] Pinecone query failed:', (error as Error).message);
    tripPineconeCircuit();
    return [];
  }
}
